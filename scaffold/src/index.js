...
function lineAnnotation(container, label, title, x, y, dx, dy, annotationWidth, font) {
...
function circleAnnotation(container, label, title, x, y, dx, dy, radius, annotationWidth, font) {
  const containerHeight = container.attr('height');
  const containerWidth = container.attr('width');
  const xScale = scaleLinear().domain([0, 100]).range([0, containerWidth]);
  const yScale = scaleLinear().domain([0, 100]).range([0, containerHeight]);
...
function rectAnnotation(container, label, title, x, y, dx, dy, width, height, annotationWidth, font) {
  const containerHeight = container.attr('height');
  const containerWidth = container.attr('width');
  const xScale = scaleLinear().domain([0, 100]).range([0, containerWidth]);
  const yScale = scaleLinear().domain([0, 100]).range([0, containerHeight]);
...
function scatterPlot(container, data, params) {
  ...
  // LEGEND
  // quicksearch
  const widthScale = scaleLinear().domain([0, plotWidth]).range([0, 100]);
  const heightScale = scaleLinear().domain([plotHeight, 0]).range([0, 100]);
  console.log(widthScale(xScale(46332)));
...
function title(container, text, textFont) {
  const height = container.attr('height');
  const width = container.attr('width');
...
function myVis(data) {
  ...
  const radialTitle = makeContainer(radialContainer, 0.4 * rWidth, 0, 0.3 * rWidth, 0.07 * rHeight, false);

  const fullScatterContainer = makeContainer(vis, 0.4 * width, 0.2 * height, 0.05 * width, 0.6 * height, false);
  const zoomScatterContainer = makeContainer(vis, 0.4 * width, 0.2 * height, 0.5 * width, 0.45 * height, false);
  const barContainer = makeContainer(vis, 0.4 * width, 0.2 * height, 0.5 * width, 0.75 * height, false);
  const scatterExplanation = makeContainer(vis, 0.4 * width, 0.1 * height, 0.05 * width, 0.5 * height, false);
  const barExplanation = makeContainer(vis, 0.4 * width, 0.1 * height, 0.05 * width, 0.83 * height, false);
  const radialExplanation = makeContainer(radialContainer, 0.4 * rWidth, 0.5 * rHeight, 0.35 * rWidth, 0.1 * rHeight, false);
  ...
  drawBar(barContainer, data[2], 'Airlines', 'Percentage', 'Which Airlines Have the Worst Delays', textFont);

  lineAnnotation(scatterExplanation, "The rightmost scatterplot has the largest 150 IATA airports (where size is measured by total number of outbound flights). The color of each dot represents the median length of a delay at that airport. Red represents a longer median delay time, while blue represents a shorter median delay time. The rightmost graph is a zoomed-in view of the first graph that shows the largest 50 airports and labels them so that you can find the airports that you frequently fly from.", "Scatterplots Explanation:", 0, 0, 0, 0, 0.4 * width, 60);

  lineAnnotation(barExplanation, "Each bar represents the percentage of delays for a particular airline. The bars are divided into two parts based on the cause for the delay. The bottom blue portion represents delays caused by the airline. The upper light yellow portion represents all other types of delays.", "Barchart Explanation:", 0, 0, 0, 0, 0.4 * width, 60);

  lineAnnotation(radialExplanation, "Each circular line in the seasons graph represents one average day in one season. The lines shooting out from the center each represent one hour in the 24-hour average day. The distance of the circular line from the center at any given point (its radius) represents the percent of delayed flights at that time of day. The fewer total flights there are at a given hour of day, the wider and less opaque the line is. The same applies to the individual season graphs by month, except each month is one line.", "Radar Explanation:", 0, 0, 0, 0, 0.3 * rWidth, 40);

  title(radialTitle, " WHEN IS THE BEST TIME TO FLY?", textFont);

  lineAnnotation(radialSeasons, "The proportion of delays gradually increases throughout waking hours of the day.", "", 57, 55, 3, 15, 400, 30);
  lineAnnotation(radialSeasons, "", "", 41, 23, 19, 47, 300, 40);
  lineAnnotation(radialSeasons, "", "", 34, 50, 26, 20, 300, 40);
  circleAnnotation(radialSeasons, "Winter has the most early morning delays.", "", 62, 28, 5, 7, 7, 300, 30);
  rectAnnotation(radialSeasons, "Fall has the lowest percentage of delayed flights.", "", 49, 80, 1, 1, 7, 16, 350, 30);

  rectAnnotation(fullScatterContainer, "Smaller airports don't appear to have a strong tendency to have longer or shorter delays.", "", 36, 80, 2, 3, 21, 70, 300, 20);
  circleAnnotation(fullScatterContainer, "Mid-sized airports have longer and more severe delays.", "", 45, 61, 7, 10, 11, 300, 20);
  rectAnnotation(fullScatterContainer, "Larger airports have the shortest delays.", "", 86, 52, 2, 3, 20, 26, 150, 20);

  lineAnnotation(zoomScatterContainer, "New York: Most New York airports have long delays, but LaGuardia has the lowest percentage of delays.", "", 51.7, 57.5, 2, 2, 300, 20);
  lineAnnotation(zoomScatterContainer, "Chicago: Midway has a higher proportion of delays, but they are much shorter.", "", 48, 17, 3, 4, 300, 20);
  // lineAnnotation(zoomScatterContainer, "", "", 48, 17, 2, 4, 300, 20);
  lineAnnotation(zoomScatterContainer, "DC: IAD has the shortest delays, but it has a higher proportion of delays overall than DCA.", "", 32, 41, -1, 0, 300, 20);
  lineAnnotation(zoomScatterContainer, "SF: Oakland has very short delays but a slightly higher proportion of delays.", "", 36, 33, 2, 2, 250, 20);
  lineAnnotation(zoomScatterContainer, "HOUSTON: HOU has much shorter delay times, but a slightly higher proportion of delays.", "", 40, 22, -1, 0, 250, 20);

  lineAnnotation(barContainer, "Virgin Airlines has a proportionally low amount of airline-related delays.", "", 42, 88, 2, 4, 300, 20);
}
